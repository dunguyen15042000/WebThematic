...
<body>
	<div class="main-content">
		...
		<form action="<c:url value='/quan-tri/binh-luan/danh-sach'/>" id="formSubmit" method="get">
			...
			<div class="row">
				<div class="col-xs-12">
					<h3 class="header smaller lighter blue">Danh sách bình luận</h3>
						...
						<table id="dynamic-table"
							...
							<thead>
								<tr>
									...
									<th class="sorting" tabindex="0" aria-controls="dynamic-table" rowspan="1" colspan="1" aria-label="Domain: activate to sort column ascending">
										<i class="ace-icon fa fa-file-o bigger-110 hidden-480"></i>Bài viết</th>
									<th class="sorting" tabindex="0" aria-controls="dynamic-table" rowspan="1" colspan="1" aria-label="Domain: activate to sort column ascending">
										<i class="ace-icon fa fa-book bigger-110 hidden-480"></i>Người bình luận</th>
									<th class="sorting" tabindex="0" aria-controls="dynamic-table" rowspan="1" colspan="1" aria-label="Domain: activate to sort column ascending">
										<i class="ace-icon fa fa-list-ul bigger-110 hidden-480"></i>Nội dung</th>
							<!-- 		<th><i class="ace-icon fa fa-adjust bigger-110 hidden-480"></i>Status</th> -->
									<th class="hidden-sm hidden-xs action-buttons">
										...
										id="btnDeleteAll" onclick="warningBeforeDeleteAll()" title="Xóa bình luận"> 
									...
									<tr>
										...
										<td>
											<div class="hidden-sm hidden-xs action-buttons">
													...
													<button type="button" class="red" style="border:none !important; background:none !important"
													  id="btnDelete" onclick="warningBeforeDelete()" title="Xóa bình luận"> 
														<i class="ace-icon fa fa-trash-o bigger-130"></i>
													</button>
											</div>
	...
	function deleteComment(data) {
     $.ajax({
         ...
         success: function (result) {
        	 console.log(result);
             window.location.href = "${commentURL}?&message=delete_success";
         },
	...
	function warningBeforeDeleteAll() {
		swal({
		  ...
		  if (isConfirm) {
				...
				deleteAllComment(ids);
		  }
		});
	} 
		function deleteAllComment(data) {
		 $.ajax({
		     url: '${commentAPI}',
		     type: 'DELETE',
		     contentType: 'application/json',
